...
// LIMITE MINÍMO DA VOLTAGEM EM OPERAÇÃO ANTES DE ENTRAR EM MODO deepSleepMax()
...
const int INTERVAL_SCAN_NETWORK = 30000;
// INTERVALO PARA ENVIAR ARQUIVO JSON AO MQTT COM TODOS OS PARAMETROS DE REDE COLETADOS
const int INTERVAL_TRANSMIT_SCANNED_NETWORK = 33000;
...
//LISTA COM TODOS OS BSSs SCANEADOS
typedef struct {
...
//#define WIFI_TX_POWER 1 // TX power of ESP module (0 -> 0.25dBm) (0...85) range [0, 82].
...
void setup() {
  ...
  //EXIBE NO MONITOR SERIAL A QUANDIDADE DE MEMÓRIA HEAP LIVRE
  heapMem();
...
// FUNÇÃO REPETE-SE ENQUANTO A ESTAÇÃO ESTIVER LIGADA
void loop() {
  ...
  if ((unsigned long)(currentMillisModule - previousMillisModule) >= INTERVAL_MODULE) {
    ...
    switch (ModuleState) {
      case ModuleIsDisconnected:
        if ((unsigned long)(currentMillis - previousMillisWiFi) >= INTERVAL_WiFi) {
          ...
          enableWiFi();
          //USADO QUANDO UM FUNÇÃO LEVA ALGUM TEMPO PARA SER CONCLUÍDA O CONTROLE É PASSADO PARA OUTRAS TAREFAS.
          yield();
    ...
    //MANTEM VIVA A CONEXÃO COM MQTT
